...
const ENDPOINT = "https://backend-z1cn.onrender.com"
...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`https://backend-z1cn.onrender.com/api/message/${selectedChat._id}`, config)
  ...
  const sendMessage = async (event) => {
    if (event.key === "Enter" && newMessage) {
      ...
      try {
        ...
        const { data } = await axios.post("https://backend-z1cn.onrender.com/api/message", {
          content: newMessage,
